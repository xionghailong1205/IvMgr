 ...
 */
function archiveDevLog(version, completedItems, content) {
    ...
    console.log(`✅ 已将 devLog 转换为备份文件: ${version}.md，并清空了 devLog.md`);
}
/**
 * 执行一次受保护的版本发布提交流程。
 *
...
export default async function commit() {
    ...
    if (inProgressItems.length > 0) {
        ...
        console.error(`\x1b[31m⛔ 未完成开发任务:\x1b[0m`);
        inProgressItems.forEach((item) => console.error(`  📝. [${typeMap[item.type]}] ${item.text}`));
        console.error(`请确认是否所有任务都完成再 commit，或从 devLog 中剔除未完成任务。`);
        process.exit(1);
    }
    ...
    const answer = await new Promise((resolve) => {
        rl.question(`🚀 即将转换成 ${version} 并执行 git 操作，是否确认发布该版本？(y/n): `, resolve);
    });
    rl.close();
