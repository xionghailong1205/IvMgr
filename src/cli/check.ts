...
export default function check(isSilent = false) {
    ...
    const completedItems: changeEntry[] = [];
    const inProgressItems: changeEntry[] = [];
    ...
    lines.forEach((line, i) => {
        ...
        if (completedMatch && completedMatch[1] && completedMatch[2]) {
            completedItems.push({ type: completedMatch[1].toUpperCase() as changeTypes, text: completedMatch[2] });
        } else if (inProgressMatch && inProgressMatch[1] && inProgressMatch[2]) {
            inProgressItems.push({ type: inProgressMatch[1].toUpperCase() as changeTypes, text: inProgressMatch[2] });
        } else {
